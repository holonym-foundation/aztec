...
import TokenPortalJson from '../../../l1-contracts/artifacts/contracts/TokenPortal.sol/TokenPortal.json'

const TokenPortalAbi = TokenPortalJson.abi
const TokenPortalBytecode = TokenPortalJson.bytecode as `0x${string}`
...
import { fetchSignature } from './helper'
...
export class CrossChainTestHarness {
  ...
  generateClaimSecret(): [Fr, Fr] {
    this.logger.debug(
      "Generating a claim secret using pedersen's hash function"
    )
  ...
  async sendTokensToPortalPublic(bridgeAmount: bigint, secretHash: Fr) {
    ...
    // Fetch the signature from the Holonym API
    const actionId = 123456789 // Replace with the actual action ID
    const userAddress = this.ethAccount.toString()

    const { isUnique, signature, circuitId } = await fetchSignature(
      actionId,
      userAddress
    )
    // const signature = "0xe19a7d0a832e434d6bbf605455046ad05c7a62ccb499a0eb321b3603da0b68762631385f7a1ae5fa6758efb9714ee29e151f1adfff0587853ddfa5250ddcf8221b"
    const attesterSig = signature
  ...
  async sendTokensToPortalPrivate(
    ...
    const actionId = 123456789 // Replace with the actual action ID
    const userAddress = this.ethAccount.toString()
    // const userAddress = '0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266' // correct address
    // const userAddress = '0xf39fd6e51aad88f6f4ce6ab8827279cfffb92268' // wrong address
  ...
  async consumeMessageOnAztecAndMintPublicly(
    ...
    await this.l2Bridge.methods
      .claim_public(this.ownerAddress, bridgeAmount, secret, leafIndex)
      .send()
      .wait()
  }
